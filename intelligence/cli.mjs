  ...
  switch (command) {
    ...
    case "seo-three-kings": {
      ...
      if (singleUrl) {
        ...
        if (audit.title?.recommendation) console.log(`           → ${audit.title.recommendation}`);
        console.log(`  H1:      [${audit.three_kings_status?.h1}] ${audit.h1?.current || "N/A"}`);
        if (audit.h1?.recommendation) console.log(`           → ${audit.h1.recommendation}`);
        console.log(`  Opening: [${audit.three_kings_status?.opening}] ${(audit.opening?.first_sentence || "N/A").slice(0, 70)}`);
        if (audit.opening?.recommendation) console.log(`           → ${audit.opening.recommendation}`);
        if (audit.url_check) {
          console.log(`  URL:     [${audit.three_kings_status?.url}] ${audit.url_check.current || "N/A"}`);
          if (audit.url_check.recommendation) console.log(`           → ${audit.url_check.recommendation}`);
          if (audit.url_check.stability_note) console.log(`           ${audit.url_check.stability_note}`);
        }
        if (audit.meta_desc) {
          console.log(`  Meta:    [${audit.three_kings_status?.meta_description}] "${(audit.meta_desc.current || "MISSING").slice(0, 60)}"`);
          if (audit.meta_desc.recommendation) console.log(`           → ${audit.meta_desc.recommendation}`);
          if (audit.meta_desc.improved_meta) console.log(`           Improved: "${audit.meta_desc.improved_meta}"`);
        }
      ...
      console.log(`  URL:    pass ${summary.by_king.url.pass} / fail ${summary.by_king.url.fail}`);
      console.log(`  Meta:   pass ${summary.by_king.meta_description.pass} / fail ${summary.by_king.meta_description.fail}`);

      audits.forEach((a, i) => {
        console.log(`\n  ${i + 1}. ${a.query} (pos ${a.average_position}, score ${a.opportunity_score})`);
        console.log(`     Title:   [${a.three_kings_status?.title}] ${a.title_check?.current?.slice(0, 65) || "N/A"}`);
        if (a.title_check?.recommendation) console.log(`              → ${a.title_check.recommendation}`);
        console.log(`     H1:      [${a.three_kings_status?.h1}] ${a.h1_check?.current?.slice(0, 65) || "N/A"}`);
        if (a.h1_check?.recommendation) console.log(`              → ${a.h1_check.recommendation}`);
        console.log(`     Opening: [${a.three_kings_status?.opening}] ${(a.opening_check?.first_sentence || "N/A").slice(0, 65)}`);
        if (a.opening_check?.recommendation) console.log(`              → ${a.opening_check.recommendation}`);
        console.log(`     URL:     [${a.three_kings_status?.url}] ${a.url_check?.current || "N/A"}`);
        if (a.url_check?.recommendation) console.log(`              → ${a.url_check.recommendation}`);
        console.log(`     Meta:    [${a.three_kings_status?.meta_description}] "${(a.meta_desc?.current || "MISSING").slice(0, 65)}"`);
        if (a.meta_desc?.improved_meta) console.log(`              Improved: "${a.meta_desc.improved_meta}"`);
